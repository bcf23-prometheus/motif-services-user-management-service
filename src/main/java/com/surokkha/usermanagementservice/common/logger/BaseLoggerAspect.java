package com.surokkha.usermanagementservice.common.logger;

import com.surokkha.usermanagementservice.common.annotations.SensitiveData;
...
public abstract class BaseLoggerAspect {
	...
	@Pointcut ("@within(com.surokkha.usermanagementservice.common.annotations.NoLogging)")
	...
	protected String serializeResponseToJson(ProceedingJoinPoint joinPoint,
	                                         Object object) throws JsonProcessingException {
		
		if (Objects.isNull(object) || doesReturnTypeHaveSensitiveAnnotation(joinPoint)) return BLANK;
		
		String response = objectMapper.writeValueAsString(object);
		
		if (response == null) response = BLANK;
		
		return String.format("%s.%s Response: %s",
		                     joinPoint.getTarget()
		                              .getClass()
		                              .getName(),
